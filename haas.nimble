...
task tests, "Running all tests":
  ...
  exec "sh mkpdf.sh \"`cat tmp_filename`\"  \"`cat tmp_filename`\" || echo 'Failed to generate PDF from test results!'"
  echo "done."
...
task bench, "Running benchmarks":
  ...
  echo ""
  echo "Generating PDF..."
  exec "sh mkpdf.sh \"`cat tmp_filename`\"  \"`cat tmp_filename`\" || echo 'Failed to generate PDF from benchmark results!'"
  echo "done."
  exec "rm tmp_filename"
